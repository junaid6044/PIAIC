...
export default function Input(props: { attr: InputProps }) {
  const commonProps = {
    ...
    className: 'w-full bg-gray-800 mb-4 pl-6 p-2 rounded-3xl sm:w-full md:w-1/3 mx-2',
    name: props.attr.name,
    id: props.attr.id,
    value: (props.attr.value === 0 && props.attr.inputType === "number") ? props.attr.placeholder : ((props.attr.value === 0 && props.attr.inputType === "tel") ? "" : props.attr.value),
